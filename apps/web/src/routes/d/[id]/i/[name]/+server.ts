...
export const GET: RequestHandler = async ({ params, request, setHeaders, locals }) => {
    ...
    const refed = db.select({ x: schema.imageRefs.documentId }).from(schema.imageRefs)
        .where(eq(schema.imageRefs.imageId, img.id)).all().some((r) => r.x === params.id);
    if (!refed) error(404, 'Not Found');
    // no-cache 协商：ETag=content_hash，命中 If-None-Match → 304 无 body
