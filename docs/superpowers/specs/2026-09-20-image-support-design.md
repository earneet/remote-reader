...
**桥版本识别（2026-09-22）**：三层机制识别/兜住无图片支持的旧桥（npm 0.1.0 的 `![x](imgs/red.png)` 原样上传 → 查看页静默裂图）——Layer ① UA 声明：`createApiClient` 增可选 `userAgent`，uploadDocument 与图片 JSON 方法（init/relay/confirm）携带 `User-Agent: remote-reader-bridge/<version>`，hooks 访问日志对 `/api/` 路径追加 ` ua=`；Layer ② 服务端内容兜底检测：`lib/server/upload-warnings.ts` 用 `extractLocalImageSrcs` 单源提取本地引用、剔除该文档已登记裸名后经响应 `warnings` 提示升级（total 全量计数 + listed 截 5 条）；Layer ③ `MIN_BRIDGE_VERSION`（`lib/server/bridge-compat.ts`，当前 0.2.0）随 `X-Remote-Reader-Min-Bridge` 响应头下发，shared `semver.ts` `compareVersions` + `uploadDocumentHandler` 自检提示升级。桥版本 bump 0.2.0（`src/version.ts` 读包根 package.json，读失败兜底 0.0.0 + stderr 记录）。审查修复（同日）：`extractLocalImageSrcs` 免行号化——不再经 `imageTokenLines` 行定位（表格上下文父 inline map 为 null 时 `lineOf` 退化为全文 findIndex，O(引用数×全文)，认证用户可用大表格+海量引用单请求阻塞事件循环），detector 路径零行扫描；warnings 计数改 `{total, listed}` 如实报全量（原 slice(0,5) 后计数低估总数）。完整记录见 `AGENTS.md`「桥版本识别」。
