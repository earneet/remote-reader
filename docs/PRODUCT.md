...
Remote Reader 把这件事变成**一个 MCP 工具调用**：Agent 调 `upload_document`，得到一个持久链接，用户在浏览器里看到带语法高亮和表格的完整渲染。

## 3. 核心场景

```
Agent                    本地 MCP 桥             Web 应用                人类用户
  │ upload_document         │                       │                       │
  │ (name, content, path)   │                       │                       │
...
3. **默认私有 + 可分享**——文档不公开遍历；上传时自动生成一把"查看钥匙"（share token），持有者可读，将来 owner 可撤销。
...
- 设计与架构：[`specs/2026-07-18-remote-reader-design.md`](./superpowers/specs/2026-07-18-remote-reader-design.md)
