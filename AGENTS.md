...
**七牛 s3 图片后端 confirm 503 修复（2026-09-23，生产排障全链路实证）**：症状——`IMAGE_STORE_BACKEND=s3` 后 confirm 恒 503「image storage unreachable」，而 init/presign/PUT 全通。根因——**七牛网关的 Range GET 响应自相矛盾**：`Content-Range` 声明分片、`content-md5` 给分片 MD5、实际流回**全量对象 body**；AWS SDK ≥3.729 默认 `responseChecksumValidation: WHEN_SUPPORTED` 在消费 body 时校验不过抛 `ChecksumMismatch` → mapGetError → 503（HEAD 无 body 不触发、PUT/relay 不读响应 body，故只有 confirm 挂）。修复——`S3BlobStore` 单源配置加 `responseChecksumValidation: 'WHEN_REQUIRED'`（`2e821e3`，钉住配置单测防回归；七牛仍流全量 body，`getRange` 返回值可能长于请求区间，confirm 只取前 32 字节不受影响）+ relay/confirm 503 分支补 `console.warn` 打 cause（`f8bb6d4`——此前 cause 被吞致生产 503 不可诊断，本次全靠沙箱外 bundle 复现才拿到真错）。**排障方法论备忘**：「同机 root shell 裸 SDK 复现成功」可能是假阴性——SDK 响应校验在**消费 body 流**时才触发，只读 `ContentRange` 不读 body 永远成功；systemd 沙箱二分用 `systemd-run --property=...` 逐项叠加。**新不变量**：S3Client 配置不得移除 WHEN_REQUIRED（七牛 confirm 即回 503）。**备案**：update.sh 的 SRC_DIR 由脚本自身路径推导——在 /opt/remote-reader 里跑 `./scripts/update.sh` 会自我 rsync 旧源码"假升级"（已发生一次），SRC==DEST 拒绝执行守卫待做。
